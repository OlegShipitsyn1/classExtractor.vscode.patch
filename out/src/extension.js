...
function replaceClassFromTemplate(cssClass) {
    var replacementTemplate = vscode.workspace.getConfiguration('class-extractor').get('outputClassFormat');
    if (replacementTemplate == null || replacementTemplate === '') {
        replacementTemplate = '@ {}';
    }
    return replacementTemplate.replace('@', cssClass);
}
function extractClassesFromMarkup(markup) {
...
function getMatchingLetterCount(firstWord, secondWord) {
...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('extension.extractClasses', () => {
        ...
        const finalString = outputClasses.reduce((classText, classToAdd) => classText + (classText !== '' ? '\n\n' : '') + replaceClassFromTemplate(`.${classToAdd}`), '');
        // Copy string to user's clipboard and show notification
        clipboardy.writeSync(finalString, () => {
...
function test(input, separator, depth = 0) {
